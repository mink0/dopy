...
function initEnv(envName, config=module.exports.config) {
  ...
  env.config.local = extend(true, {}, env.config.general, env.config.local);
  env.config.remote = extend(true, {}, env.config.general, env.config.remote);
  delete env.config.general;

  // force array
  if (env.config.remote.servers && !Array.isArray(env.config.remote.servers))
    env.config.remote.servers = [env.config.remote.servers];
...
function initTargets(selectedTargets) {
  ...
  Object.keys(env.config.targets).forEach(t => {
    ...
    target.local = extend(true, {}, target.general, target.local);
    target.remote = extend(true, {}, target.general, target.remote);
    delete target.general;

    target.local = extend(true, {}, env.config.local, target.local);
    target.remote = extend(true, {}, env.config.remote, target.remote);
  });
